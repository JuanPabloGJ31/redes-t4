...
class Enlace:
    ...
    def enviar(self, datagrama):
        ...
        datagrama = datagrama.replace(bytes.fromhex("DB"), bytes.fromhex("DBDD")).replace(
            bytes.fromhex("C0"), bytes.fromhex("DBDC"))
        ...
        if bytes.fromhex('C0') in dados:
            ...
            for datagrama in dados:
                if datagrama !=b'':
                    ...
                    datagrama = datagrama.replace(bytes.fromhex("DBDD"), bytes.fromhex("DB"))
                    try:
                        self.callback(datagrama)
                    except:
                        pass
